
fetchActivePlayers();
fetchLeaderboardData();

function fetchActivePlayers() {
    const socket = new WebSocket('ws://' + location.host + '/players');
    //while(true) {
            // reset data
            $('#playerContainer').html('')
            text = '<li><a class="dropdown-item d-flex align-items-center gap-2 py-2" href="#">'
                +'<span class="d-inline-block bg-success rounded-circle" style="width: .5em; height: .5em;"></span>plyr'
                +'<span class="d-inline-block"> Location: loctn</span></a></li>'
            socket.addEventListener('message', ev => {
                data = JSON.parse(ev.data)
                replacedText = text.replace('plyr', data.username)
                replacedText = replacedText.replace('loctn', data.location)
                $('#playerContainer').append(replacedText)
            });
    //}
}

function fetchLeaderboardData() {
    const socket2 = new WebSocket('ws://' + location.host + '/game/01/leaderboard');
    //while(true) {
            // reset data
            $('#leaderboardContainer').html('')
            text = '<li><a class="dropdown-item d-flex align-items-center gap-2 py-2" href="#"><span class="d-inline-block bg-success rounded-circle" style="width: .5em; height: .5em;"></span>player<span class="badge rounded-pill bg-info text-dark"> score</span></a></li>'

            //<span class="badge rounded-pill bg-info text-dark">Info</span>

            socket2.addEventListener('message', ev => {
                data = JSON.parse(ev.data)
                replacedText = text.replace('player', data.player)
                replacedText = replacedText.replace('score', data.score)
                $('#leaderboardContainer').append(replacedText)
            });
    //}
}
